...
const blocks: Block[] = [
  { key: 'identite', icon: '◆', label: "Le ton du média", help: "La personnalité du rédacteur IA (le nom se règle au-dessus) : direct, scandalisé, implacable. C'est ici qu'on lui donne sa ligne : mettre en avant le deux poids deux mesures, citer les paroles exactes de la droite, dénoncer les sous-entendus, et rester aligné avec la gauche sociale (LFI, NPA) qu'il ne faut pas attaquer.", resetTo: FACTORY_PROMPTS.identite, get: getKey('identite'), set: setKey('identite') },
  { key: 'mission', icon: '➤', label: "L'enquête avant d'écrire", help: "Ce que l'IA fait avant de rédiger : lire les articles fournis, vérifier les faits sur internet, chercher la contradiction « deux poids deux mesures », retrouver les paroles exactes des politiques de droite pour les citer, et débusquer leurs « casseroles ».", resetTo: FACTORY_PROMPTS.mission, get: getKey('mission'), set: setKey('mission') },
  ...
  { key: 'consigneTri', icon: '⚖', label: "Ce que le robot garde au tri", help: "Le filtre du tri automatique : la ligne éditoriale du média (populaire, marxiste, panafricaniste, anti-impérialiste) — la politique française vue d'en bas, l'anti-impérialisme (Trump, Israël/Palestine, la France en Afrique), les luttes sociales et écologiques. Plus la méfiance automatique quand une source de droite ou d'extrême droite attaque la gauche.", resetTo: FACTORY_PROMPTS.consigneTri, get: getKey('consigneTri'), set: setKey('consigneTri') },
  { key: 'criteresRejet', icon: '✕', label: "Ce que le robot jette", help: "Les sujets écartés d'office au tri : les infos internationales anecdotiques sans enjeu (monarchies, faits divers locaux étrangers, sport…), les faits divers isolés (sauf injustice systémique), le lifestyle et la culture people, les polémiques stériles, la communication gouvernementale.", resetTo: FACTORY_PROMPTS.criteresRejet, get: getKey('criteresRejet'), set: setKey('criteresRejet') },
