...
export async function generateMetadata({ params }: { params: { ville: string } }): Promise<Metadata> {
    const villeName = formatVille(params.ville);
    ...
    return {
        ...
        description,
        alternates: {
            canonical: `https://lassez.fr/elections/municipales-2026/${params.ville}`,
        },
        openGraph: {
            title,
            description,
            type: 'article',
            url: `https://lassez.fr/elections/municipales-2026/${params.ville}`,
            images: [
                {
...
export default function VilleElectionPage({ 
    params, 
    searchParams 
}: { 
    params: { ville: string },
    ...
    const villeDisplay = formatVille(params.ville);
    const initialDep = searchParams.dep || '';
    ...
    const breadcrumbSchema = {
        ...
        "itemListElement": [
            ...
            { "@type": "ListItem", "position": 3, "name": villeDisplay, "item": `https://lassez.fr/elections/municipales-2026/${params.ville}` },
        ]
    };

    return (
        <Layout>
            <Script id={`json-ld-breadcrumb-${params.ville}`} type="application/ld+json" strategy="afterInteractive">
