<template>
  <div class="space-y-5">
    ...
    <LCard title="🎟️ Niveau gratuit — la répartition des modèles" :description="`Tes 3 nœuds IA tournent sur ${modelNames} — rien de plus cher n'est utilisé`">
      <div class="grid sm:grid-cols-3 gap-3">
        <div class="rounded border border-border/60 bg-bg/40 px-3 py-2.5">
          <p class="text-[10px] uppercase tracking-wider text-text-3 mb-1">🧭 Orchestrateur / Tri</p>
          <p class="text-sm font-medium text-text-1">{{ store.ecriture.modeleOrchestrateur }}</p>
          <p class="text-[10px] text-text-3 mt-1">Chef de desk — 1 appel/cycle · thinking moyen</p>
        </div>
        <div class="rounded border border-border/60 bg-bg/40 px-3 py-2.5">
    ...
    <!-- La chaîne visuelle -->
    <LCard :padding="false" title="La chaîne d'un coup d'œil" description="8 nœuds — 7 dans le cycle principal, la diffusion tourne en continu à part">
    ...
    <LCard title="💡 À retenir">
      <ul class="space-y-1.5 text-xs text-text-2 list-disc list-inside">
        ...
        <li><b>Sans clé Gemini</b>, les nœuds Orchestrateur / Tri / Rédaction / Vérification sont ignorés proprement (les sujets restent en attente) — la clé se règle dans <router-link to="/systeme" class="text-accent hover:underline">Système → Clé API Gemini</router-link>.</li>
...
const modelNames = computed(() => {
  const seen = new Set([store.ecriture.modeleOrchestrateur, store.ecriture.modeleRapide, store.ecriture.modeleRedaction, store.ecriture.modeleVerification])
...
const chain = computed(() => [
  ...
  {
    id: 3, type: 'orchestrator', icon: '✸', label: 'Orchestrateur',
    role: 'Le chef de desk : lit tout le cycle, fixe l’agenda et aiguille chaque sujet',
    what: 'En un SEUL appel IA par cycle, il lit la liste complète des sujets ingérés et décide pour chacun : à traiter (avec le format, la zone et l’angle) ou à écarter. C’est lui qui fixe l’agenda du jour — quand il est activé, il remplace le Tri. S’il échoue (quota, clé), les sujets restent en attente et le Tri reprend automatiquement.',
    input: 'Tous les sujets ingérés du cycle (max 50 par passage).',
    output: 'Sujets aiguillés (statut RESEARCHED, avec format + zone) ou écartés (REJECTED).',
    settingsTo: '/atelier',
    settings: [
      `Modèle : ${store.ecriture.modeleOrchestrateur}`,
      `Raisonnement : ${store.ecriture.thinkingOrchestrateur} tokens (moyen)`,
      `Sujets par cycle : ${store.ecriture.maxSujetsOrchestrateur}`,
    ],
    model: store.ecriture.modeleOrchestrateur,
    temp: '0.1 (strict)',
    note: 'Régler dans Atelier → Orchestrateur.',
    inputCount: { label: 'En attente (INGESTED)', value: String(counts.value.INGESTED ?? 0) },
    outputCount: { label: 'Aiguillés (RESEARCHED)', value: String(counts.value.RESEARCHED ?? 0) },
  },
  {
    id: 4, type: 'research', icon: '🧠', label: 'Tri',
  ...
  {
    id: 5, type: 'editor', icon: '✍️', label: 'Rédaction',
  ...
  {
    id: 6, type: 'validator', icon: '⚖️', label: 'Vérification',
  ...
  {
    id: 7, type: 'media', icon: '📸', label: 'Image',
  ...
  {
    id: 8, type: 'publisher', icon: '📤', label: 'Diffusion',
