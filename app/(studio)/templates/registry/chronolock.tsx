        ...
        return (
            <div className="w-full h-full bg-[#0F0F0F] overflow-hidden border-4 border-black flex flex-col relative">
                    ...
                    {timeline.map((ev: any, i: number) => (
                        <div key={i} className="flex gap-5 relative">
                            ...
                            <div className="flex-1 pb-6">
                                <div className="inline-block px-2 py-0.5 sm font-black uppercase text-white mb-1.5" style={{ backgroundColor: i === 0 ? state.accent : '#1A1C1C', fontSize: '0.6rem', letterSpacing: '0.12em' }}>{ev.date}</div>
                                <p className="ab font-black uppercase text-white leading-tight mb-1" style={{ fontSize: '1rem' }}>{ev.event}</p>
                                <p className="sm text-white/60 font-bold uppercase" style={{ fontSize: '0.62rem' }}>{ev.impact}</p>
                            </div>
                        </div>
                    ))}
                </div>
                ...
                <div className="bg-black text-white px-6 py-3 border-t-4 border-black shrink-0 flex items-center justify-between">
                    <span className="ab font-black uppercase text-white tracking-wide" style={{ fontSize: '0.65rem' }}>{state.brand} / RIEN N'EST UN HASARD</span>
                </div>
            </div>
        );
    }
