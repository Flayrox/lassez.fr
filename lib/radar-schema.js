...
export const RADAR_SCHEMA = {
    ...
    `,
    radar_source_health: `
        CREATE TABLE IF NOT EXISTS radar_source_health (
            url TEXT PRIMARY KEY,
            type TEXT NOT NULL,
            consecutive_failures INTEGER DEFAULT 0,
            last_status INTEGER,
            last_error TEXT,
            status TEXT DEFAULT 'HEALTHY' CHECK(status IN ('HEALTHY', 'DEGRADED', 'DISABLED')),
...
export function syncDatabase(db) {
    ...
    try {
        db.transaction(() => {
            ...
            // 4. Safe Migrations: system_health
            const healthCols = db.pragma('table_info(system_health)').map(c => c.name);
            if (!healthCols.includes('details')) {
                console.log('  ↳ [MIGRATION] Adding column "details" to "system_health"');
                db.exec('ALTER TABLE system_health ADD COLUMN details TEXT');
            }

            // 5. Automatic updated_at trigger for radar_jobs
            db.exec(`
            ...
            db.exec(`
                CREATE TRIGGER IF NOT EXISTS update_radar_source_health_timestamp
                AFTER UPDATE ON radar_source_health
                FOR EACH ROW
                BEGIN
                    UPDATE radar_source_health SET updated_at = CURRENT_TIMESTAMP WHERE url = OLD.url;
                END;
            `);
