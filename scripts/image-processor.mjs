...
// Dossier public de Next.js (racine du repo, pas le dossier du script)
...
// Settings lus depuis le global Payload radar-settings (ex table SQLite
// radar_settings). PAYLOAD_API_URL + identifiants bot dans l'environnement.
async function getImageEngineSettings() {
    ...
    try {
        const base = (process.env.PAYLOAD_API_URL || process.env.PAYLOAD_SERVER_URL || 'http://localhost:5173')
            .replace(/\/+$/, '').replace(/\/api\/payload$/, '') + '/api/payload';
        const email = process.env.PAYLOAD_BOT_EMAIL || 'bot@lassez.fr';
        const password = process.env.PAYLOAD_BOT_PASSWORD || '';
        if (!password) return result;
        const loginRes = await axios.post(`${base}/authors/login`, { email, password }, { timeout: 15000 });
        const token = loginRes.data?.token;
        if (!token) return result;
        const res = await axios.get(`${base}/globals/radar-settings`, {
        ...
        result.image_overlay_enabled = s.image_overlay_enabled !== undefined ? Boolean(s.image_overlay_enabled) : result.image_overlay_enabled;
        result.image_overlay_opacity = num(s.image_overlay_opacity, result.image_overlay_opacity);
        result.image_box_scale_169 = num(s.image_box_scale_169, result.image_box_scale_169);
        result.image_box_scale_1x1 = num(s.image_box_scale_1x1, result.image_box_scale_1x1);
    } catch (e) {
        console.warn('[PROCESSOR] Settings image indisponibles, fallback par défaut.', e?.message);
    }
    return result;
}
...
export async function generateSmartCacheImage(sourceImageUrl, keywordFallback, articleTitle = '', subtext = '') {
    ...
    try {
        const engineSettings = await getImageEngineSettings();
...
// ===== Mode CLI =====
// Usage : node scripts/image-processor.mjs --title "..." [--image URL] [--keyword kw] [--subtext "..."]
...
if (isMain) {
    ...
    const title = get('--title') || 'LA COLÈRE GRONDE DANS LES RUES';
    const imageUrl = get('--image') || '';
    const keyword = get('--keyword') || title;
    const subtext = get('--subtext') || '';
    console.log(`[PROCESSOR] Génération du visuel « ${title} »`);
    generateSmartCacheImage(imageUrl || null, keyword, title, subtext).then((res) => {
        if (res) console.log(`✅ ${res.publicUrl}`);
